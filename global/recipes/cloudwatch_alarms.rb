...
['cpu', 'mem', 'disk'].each do |check|
  ...
  execute check do
    command "aws cloudwatch put-metric-alarm --region #{AWSRegion} --alarm-name #{AlarmName} --metric-name #{MetricName} --namespace #{NameSpace} --statistic #{Statistic} --unit #{Unit} --period #{Period} --threshold #{Threshold} --comparison-operator #{ComparisonOperator} --dimensions #{Dimensions} --evaluation-periods #{EvaluationPeriods} --ok-actions #{Actions} --alarm-actions #{Actions}"
  end
end
